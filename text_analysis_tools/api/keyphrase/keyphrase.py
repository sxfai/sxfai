...
#from text_analysis_tools.text_analysis_tools.api.keyphrase import FONT_PATH, MASK_PATH
...
class KeyPhraseExtraction():
    ...
    def key_phrase_extraction(self, text):
        ...
        print('cut_sentence is ', cut_sentences)
        for sent in cut_sentences:
            ...
            for word in sent:
                if word in keywords:
                   ...
                   # print('word not in keywords, word is ', word)
                    if len(temp) > 1:
                        if temp not in key_phrase:
                            key_phrase.append(temp)
                            #print('len temp > 1 is ', temp)
                            #print('key_phrase is', key_phrase[-1])
                        print('key_phrase is', key_phrase[-1])
                    temp = []
        ...
        for phrase in key_phrase:
            ...
            if not flag:
                    ...
                    print('key_phrase_filter is', key_phrase_filter)
...
if __name__ == '__main__':
    with open('/Users/sixuefeng/Desktop/nlp_text_data.txt', encoding='utf-8') as f:
        text = f.read()
    key_phrase_extractor = KeyPhraseExtraction(topk=30, method='tfidf')
